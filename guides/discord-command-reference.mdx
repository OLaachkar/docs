...
<Note>
  Discord activity and presence editing are not plan-locked. The current
  plan-gated Discord controls are the voice channel join/leave commands.
</Note>
...
| `/vc-join` | Server | `Ultra` plan + voice engine enabled + voice channel access |
| `/vc-leave` | Server | `Ultra` plan + voice engine enabled |
...
- Available on the `Ultra` plan in the current release.

### Natural-language voice join

- Ultra bots can also join from a normal mention message when the message is
  clearly asking them to join VC/call.
