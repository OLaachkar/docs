...
All of these are enabled by default for new bots. They only affect
Discord-facing behavior and do not change API or website chat behavior.
...
<Note>
  In the current release, Discord activity/presence editing is available on all
  plans. The voice channel join/leave commands are reserved for `Ultra`.
</Note>

## Voice join without slash commands

Ultra verbs do not need `/vc-join` every time. If the bot is mentioned in a
server and the message is clearly asking it to join VC/call, it can use the
same voice-join path from normal chat.
...
- Voice commands require Voice Engine enabled for the verb, and `/vc-join` plus
  `/vc-leave` are Ultra-only in the current dashboard release.
